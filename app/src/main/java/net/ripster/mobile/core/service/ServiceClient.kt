...
interface ServiceClient {
    ...
    /**
     * Полный состав трека — кто ещё записан, кроме одного имени в выдаче.
     *
     * `null` — сервис молчит или клиент так не умеет. Зовётся ЛЕНИВО и только по
     * тому, что человек видит или слушает ([ArtistDepth]): у Deezer и Qobuz в
     * поисковой выдаче у трека одно имя, и остальной состав достаётся отдельным
     * запросом к карточке. Дёргать его для всей страницы выдачи — значило бы
     * устроить двадцать запросов там, где человек прочитал четыре строки.
     */
    suspend fun contributorsFor(track: Track): Contributors? = null
