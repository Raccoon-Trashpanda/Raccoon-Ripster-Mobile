...
data class Album(
    ...
    val artworkUrl: String? = null,
    val upc: String? = null,
    // Каноническая ссылка на релиз, если сервис её отдал. Строить ссылку из id
    // получается не у всех (у плейлиста SoundCloud нужен постоянный адрес, а у
    // Apple номер релиза свой в каждой витрине), поэтому лучше сохранить ту,
    // что пришла в ответе, чем собирать её потом на догадках.
    val url: String? = null,
...
data class Track(
    ...
    val releaseDate: String? = null,
    /**
     * Насколько вещь слушают, 0..1. Заполняется ТОЛЬКО из настоящих
     * счётчиков сервиса (`rank` у Deezer, число прослушиваний у
     * SoundCloud). `null` — сервис такого не отдаёт; это «не знаю», а не
     * «непопулярно», и в подборе такие вещи не наказываются.
     */
