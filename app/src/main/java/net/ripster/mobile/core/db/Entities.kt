...
data class DownloadEntity(
    ...
    val updatedAt: Long,
    /**
     * К какому релизу относится задача. null — одиночный трек.
     *
     * Просьба владельца 06.09.2026: «если нажать скачать альбом, надо качать
     * их в одной подгруппе с возможностью развернуть релиз». До этого альбом
     * из тринадцати треков давал тринадцать одинаковых строк, и очередь
     * переставала читаться.
     */
    val groupId: String? = null,
    /** Название релиза для шапки группы. Живёт рядом с [groupId]. */
