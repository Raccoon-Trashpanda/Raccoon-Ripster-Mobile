...
object QobuzBundle {
    ...
    /** Зашитый запасной на случай, когда скрейп bundle.js не прошёл/не успел
     *  (медленный канал). Для `catalog/search` хватает валидного `X-App-Id` —
     *  подпись (секрет) нужна только для `getFileUrl`. `798273057` — публичный
     *  app_id веб-плеера (тот же, что в ПК-Ripster для поиска без авторизации),
     *  живёт месяцами; если протухнет — `get()` поймает 400 и до-скрейпит. */
    val FALLBACK = Creds(
        "798273057",
        listOf("979549437fcc4a3faad4867b5cd25dcb", "abb21364945c0583309667d13ca3d93a"),
    )
